            ...
            if (links.length != 0) {
               if (users.limit > 0) {
                  ...
                  if (users.limit >= limit) {
                     users.limit -= limit
                  } else return client.reply(m.chat, Func.texted('audacieux', `🚩Votre limite n'est pas suffisant pour utiliser cette fonctionnalité.`), m)
               }
               ...
               links.map(async link => {
                  ...
                  for (let v of json.data) {
                     ...
                     client.sendFile(m.chat, v.url, Func.filename(/mp4|bin/.test(file.extension) ? 'mp4' : 'jpg'), `🍟 *Aller chercher* : ${((new Date - old) * 1)} ms`, m)
