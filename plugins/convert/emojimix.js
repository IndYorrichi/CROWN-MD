exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!emo1 || !emo2) return client.reply(m.chat, Func.texted('bold', `🚩 Donnez 2 emoji à mélanger.`), m)
         const json = await Api.neoxr('/emoji', {
            q: emo1 + '_' + emo2
         })
         if (!json.status) return client.reply(m.chat, Func.texted('bold', `🚩 Les Emoji ne peuvent pas être mélangés.`), m)
