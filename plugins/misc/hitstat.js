...
exports.run = {
   ...
   async: async (m, {
      ...
      if (stat.length == 0) return client.reply(true, Func.texted('bold', `🚩 Aucune commande utilisée.`), m)
      ...
      teks += Func.texted('bold', `“Statistiques totales d'exécution des commandes ${command == 'hitstat' ? 'sont en ce moment' : 'pour aujourdhui'} ${Func.formatNumber(command == 'hitstat' ? sum.total('hitstat') : sum.total('today'))} hits.”`) + '\n\n'
